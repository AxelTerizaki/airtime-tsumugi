...
SQL;
    ...
    */
    ...
    public function storeCriteriaIntoDb($p_criteriaData)
    {
        ...
        // insert LOGOSTART info
        $qry = new CcBlockcriteria();
        $qry->setDbCriteria("logoStart")
        ->setDbModifier("N/A")
        ->setDbValue($p_criteriaData['etc']['sp_logoStart'])
        ->setDbBlockId($this->id)
        ->save();

	// insert JINGLESRANDOM info
        $qry = new CcBlockcriteria();
        $qry->setDbCriteria("jinglesRandom")
        ->setDbModifier("N/A")
        ->setDbValue($p_criteriaData['etc']['sp_jinglesRandom'])
        ->setDbBlockId($this->id)
        ->save();

        // insert JINGLESSTART info
        $qry = new CcBlockcriteria();
        $qry->setDbCriteria("jinglesStart")
        ->setDbModifier("N/A")
        ->setDbValue($p_criteriaData['etc']['sp_jinglesStart'])
        ->setDbBlockId($this->id)
        ->save();

	// insert LOGO info
        $qry = new CcBlockcriteria();
        $qry->setDbCriteria("logo")
	->setDbModifier($p_criteriaData['etc']['sp_logo_options'])
        ->setDbValue($p_criteriaData['etc']['sp_logo_value'])
        ->setDbBlockId($this->id)
        ->save();

        // insert JINGLES info
        $qry = new CcBlockcriteria();
        $qry->setDbCriteria("jingles")
	    ->setDbModifier($p_criteriaData['etc']['sp_jingles_options'])
        ->setDbValue($p_criteriaData['etc']['sp_jingles_value'])
        ->setDbBlockId($this->id)
        ->save();

        // insert SPOTSSTART info
        $qry = new CcBlockcriteria();
        $qry->setDbCriteria("spotsStart")
        ->setDbModifier("N/A")
        ->setDbValue($p_criteriaData['etc']['sp_spotsStart'])
        ->setDbBlockId($this->id)
        ->save();

        // insert spots info
        $qry = new CcBlockcriteria();
        $qry->setDbCriteria("spots")
        ->setDbModifier($p_criteriaData['etc']['sp_spots_options'])
        ->setDbValue($p_criteriaData['etc']['sp_spots_value'])
        ->setDbBlockId($this->id)
        ->save();

	// insert LOGOJINGLES info
        $qry = new CcBlockcriteria();
        $qry->setDbCriteria("logoJingles")
        ->setDbModifier("N/A")
        ->setDbValue($p_criteriaData['etc']['sp_logoJingles'])
        ->setDbBlockId($this->id)
        ->save();

	// insert LOGOSPOTS info
        $qry = new CcBlockcriteria();
        $qry->setDbCriteria("logoSpots")
        ->setDbModifier("N/A")
        ->setDbValue($p_criteriaData['etc']['sp_logoSpots'])
        ->setDbBlockId($this->id)
        ->save();    
	...
	if (isset($storedCrit['logo'])) {
                $logoOption = $storedCrit['logo']['modifier'];
        }

	if (isset($storedCrit['jingles'])) {
                $jingleOption = $storedCrit['jingles']['modifier'];
        }

	if (isset($storedCrit['spots'])) {
                $spotOption = $storedCrit['spots']['modifier'];
        }

	$logoList = $this->getAllLogos($logoOption);
        $jinglesList = $this->getAllJingles($jingleOption);
        $spotsList = $this->getAllSpots($spotOption);
	$counterLogo = 0;
        $counterJingles = 0;
        $counterSpots = 0;
	$logoCnt = 0;
        ...
        $spotCnt = 0;
	if (isset($storedCrit['logo'])) {
                $logoCnt = $storedCrit['logo']['value'];
        }
        if (isset($storedCrit['jingles'])) {
                $jingleCnt = $storedCrit['jingles']['value'];
        }
        if (isset($storedCrit['spots'])) {
                $spotCnt = $storedCrit['spots']['value'];
        }


	if (isset($storedCrit['logoStart'])) {
                if ($storedCrit['logoStart']['value'] == 1) {
                        foreach ($logoList as $logo) {
	                        $length = Application_Common_DateHelper::calculateLengthInSeconds($logo['length']);
      		                $insertList[] = array('id'=>$logo['id'], 'length'=>$length);
              		        $totalTime += $length;
			}
                }
        }

        if (isset($storedCrit['jinglesStart'])) {
                if ($storedCrit['jinglesStart']['value'] == 1) {
                        if (isset($storedCrit['jinglesRandom'])) {
                                if ($storedCrit['jinglesRandom']['value'] == 1 && count($jinglesList)>0) {
                                        $random = array_rand($jinglesList, 1);
                                        $length = Application_Common_DateHelper::calculateLengthInSeconds($jinglesList[$random]['length']);
                                        $insertList[] = array('id'=>$jinglesList[$random]['id'], 'length'=>$length);
                                        $totalTime += $length;
                                } else {
	                        	foreach ($jinglesList as $jingle) {
        	                        	$length = Application_Common_DateHelper::calculateLengthInSeconds($jingle['length']);
                	                	$insertList[] = array('id'=>$jingle['id'], 'length'=>$length);
                        	        	$totalTime += $length;
                        		}
                                }
                        }

			if (isset($storedCrit['logoJingles'])) {
        	                if ($storedCrit['logoJingles']['value'] == 1) {
	                                foreach ($logoList as $logo) {
                                        	$length = Application_Common_DateHelper::calculateLengthInSeconds($logo['length']);
                                        	$insertList[] = array('id'=>$logo['id'], 'length'=>$length);
                                        	$totalTime += $length;
        ...
        if (isset($storedCrit['spotsStart'])) {
                if ($storedCrit['spotsStart']['value'] == 1) {
                        foreach ($spotsList as $spot) {
                                $length = Application_Common_DateHelper::calculateLengthInSeconds($spot['length']);
                                $insertList[] = array('id'=>$spot['id'], 'length'=>$length);
                                $totalTime += $length;
                        }

			if (isset($storedCrit['logoSpots'])) {
                 	       if ($storedCrit['logoSpots']['value'] == 1) {
                        	        foreach ($logoList as $logo) {
                                	        $length = Application_Common_DateHelper::calculateLengthInSeconds($logo['length']);
                                        	$insertList[] = array('id'=>$logo['id'], 'length'=>$length);
	                                        $totalTime += $length;
        ...
        while ($iterator->valid()) {
        $diff =  $limit['time'] - $totalTime;

	    if ($logoCnt > 0 && $diff > 600) {
                if ($counterLogo == $logoCnt) {
			foreach ($logoList as $logo) {
                                $length = Application_Common_DateHelper::calculateLengthInSeconds($logo['length']);
                                $insertList[] = array('id'=>$logo['id'], 'length'=>$length);
                                $totalTime += $length;
            ...
            if ($jingleCnt > 0 && $diff > 600) {
                if ($counterJingles == $jingleCnt) {
			if (isset($storedCrit['jinglesRandom'])) {
                                if ($storedCrit['jinglesRandom']['value'] == 1 && count($jinglesList)>0) {
                                        $random = array_rand($jinglesList, 1);
                                        $length = Application_Common_DateHelper::calculateLengthInSeconds($jinglesList[$random]['length']);
            ...
            if ($spotCnt > 0 && $diff > 1200) {
                if ($counterSpots == $spotCnt) {
                        foreach ($spotsList as $spot) {
                                $length = Application_Common_DateHelper::calculateLengthInSeconds($spot['length']);
                                $insertList[] = array('id'=>$spot['id'], 'length'=>$length);
                                $totalTime += $length;
            ...
            $flp = $iterator->current()->getDbLPtime();
 ...
 foreach ($out as $crit) {
	    ...
	    } else if($criteria == "logoStart") {
                $storedCrit["logoStart"] = array("value"=>$value);
	    } else if ($criteria == "logo") {
                    $storedCrit["logo"] = array(
                    "value"=>$value,
                    "modifier"=>$modifier,
                    "display_modifier"=>_($modifier));
	    } else if($criteria == "jinglesRandom") {
                $storedCrit["jinglesRandom"] = array("value"=>$value);
            } else if($criteria == "jinglesStart") {
                $storedCrit["jinglesStart"] = array("value"=>$value);
            } else if ($criteria == "jingles") {
                    $storedCrit["jingles"] = array(
                    "value"=>$value,
                    "modifier"=>$modifier,
                    "display_modifier"=>_($modifier));
            } else if($criteria == "spotsStart") {
                $storedCrit["spotsStart"] = array("value"=>$value);
            } else if ($criteria == "spots") {
                    $storedCrit["spots"] = array(
                    "value"=>$value,
                    "modifier"=>$modifier,
                    "display_modifier"=>_($modifier));
	    } else if($criteria == "logoJingles") {
                $storedCrit["logoJingles"] = array("value"=>$value);
	    } else if($criteria == "logoSpots") {
                $storedCrit["logoSpots"] = array("value"=>$value);
                ...
                foreach ($crit as $criteria) {
                    ...
                    try {
                        ...
                        if ($spCriteriaModifier == Criteria::NOT_ILIKE || $spCriteriaModifier == Criteria::NOT_EQUAL) {
                            $qry->add($spCriteria, null, Criteria::ISNULL);
                        }
                    } catch (Exception $e) {
                        Logging::info($e);
                    }
...
SQL;
	...
	public static function getAllLogos($logo)
    {
        $sql = <<<SQL
SELECT id,length,cuein,cueout
FROM cc_files
WHERE genre like '%$logo%'
SQL;
    ...
    public static function getAllJingles($jingle)
    {
        $sql = <<<SQL
SELECT id,length,cuein,cueout
FROM cc_files
WHERE genre like '%$jingle%'
SQL;
    ...
    public static function getAllSpots($spot)
    {
        $sql = <<<SQL
SELECT id,length,cuein,cueout
FROM cc_files
WHERE genre like '%$spot%'
SQL;
