...
class Application_Form_SmartBlockCriteria extends Zend_Form_SubForm
{
	...
	private $logoOptions;
    private $jinglesOptions;
    private $spotsOptions;
...
SQL;
	...
	if (count($array)==0){
                if(!isset($this->logoOptions)){
                        $this->logoOptions = array(
                                "Logo"   => _("Logo")
                        );
    ...
    private function getJinglesOptions()
    {
...
SQL;
	...
	if (count($array)==0){
		if(!isset($this->jinglesOptions)){
			$this->jinglesOptions = array(
                      		"Jingle"   => _("Jingle")
              		);

			return $this->jinglesOptions;
		}
	}
                return $array;
    }
    ...
    private function getSpotsOptions()
    {
 $sql = <<<SQL
SELECT distinct genre, genre
FROM cc_files
...
SQL;
	...
	$logoStart = new Zend_Form_Element_CheckBox('sp_logoStart');
        $logoStart->setLabel(_('Logo At Start'))
                   ->setDecorators(array('viewHelper'));
        if (isset($storedCrit["logoStart"])) {
            $logoStart->setChecked($storedCrit["logoStart"]["value"] == 1?true:false);
        }
        $this->addElement($logoStart);

	$logo = new Zend_Form_Element_Text('sp_logo_value');
        $logo->setAttrib('class', 'sp_input_text_limit')
                   ->setLabel(_('Logo Every xx items (0 disable)'))
                   ->setDecorators(array('viewHelper'));
        $this->addElement($logo);
        if (isset($storedCrit["logo"])) {
            $logo->setValue($storedCrit["logo"]["value"]);
        } else {
           $logo->setValue(0);
        }

	$logoOption = new Zend_Form_Element_Select('sp_logo_options');
        $logoOption->setAttrib('class', 'sp_input_select')
              ->setDecorators(array('viewHelper'))
              ->setMultiOptions($this->getLogoOptions());
        if (isset($storedCrit["logo"])) {
            $logoOption->setValue($storedCrit["logo"]["modifier"]);
        }
        $this->addElement($logoOption);

	$jinglesRandom = new Zend_Form_Element_CheckBox('sp_jinglesRandom');
        $jinglesRandom->setLabel(_('Only one random Jingle'))
                   ->setDecorators(array('viewHelper'));
        if (isset($storedCrit["jinglesRandom"])) {
            $jinglesRandom->setChecked($storedCrit["jinglesRandom"]["value"] == 1?true:false);
        }
        $this->addElement($jinglesRandom);

	$jinglesStart = new Zend_Form_Element_CheckBox('sp_jinglesStart');
        $jinglesStart->setLabel(_('Jingles At Start'))
                   ->setDecorators(array('viewHelper'));
        if (isset($storedCrit["jinglesStart"])) {
            $jinglesStart->setChecked($storedCrit["jinglesStart"]["value"] == 1?true:false);
        }
        $this->addElement($jinglesStart);

        $jingles = new Zend_Form_Element_Text('sp_jingles_value');
        $jingles->setAttrib('class', 'sp_input_text_limit')
                   ->setLabel(_('Jingles Every xx items (0 disable)'))
                   ->setDecorators(array('viewHelper'));
        $this->addElement($jingles);
        if (isset($storedCrit["jingles"])) {
            $jingles->setValue($storedCrit["jingles"]["value"]);
        } else {
           $jingles->setValue(0);
        }

	$jinglesOption = new Zend_Form_Element_Select('sp_jingles_options');
        $jinglesOption->setAttrib('class', 'sp_input_select')
              ->setDecorators(array('viewHelper'))
              ->setMultiOptions($this->getJinglesOptions());
        if (isset($storedCrit["jingles"])) {
            $jinglesOption->setValue($storedCrit["jingles"]["modifier"]);
        }
        $this->addElement($jinglesOption);

	$spotsStart = new Zend_Form_Element_CheckBox('sp_spotsStart');
        $spotsStart->setLabel(_('Spots At Start'))
                   ->setDecorators(array('viewHelper'));
        if (isset($storedCrit["spotsStart"])) {
            $spotsStart->setChecked($storedCrit["spotsStart"]["value"] == 1?true:false);
        }
        $this->addElement($spotsStart);

        $spots = new Zend_Form_Element_Text('sp_spots_value');
        $spots->setAttrib('class', 'sp_input_text_limit')
                   ->setLabel(_('Spots Every xx items (0 disable)'))
                   ->setDecorators(array('viewHelper'));
        $this->addElement($spots);
        if (isset($storedCrit["spots"])) {
            $spots->setValue($storedCrit["spots"]["value"]);
        } else {
           $spots->setValue(0);
        }

	$spotsOption = new Zend_Form_Element_Select('sp_spots_options');
        $spotsOption->setAttrib('class', 'sp_input_select')
              ->setDecorators(array('viewHelper'))
              ->setMultiOptions($this->getSpotsOptions());
        if (isset($storedCrit["spots"])) {
            $spotsOption->setValue($storedCrit["spots"]["modifier"]);
        }
        $this->addElement($spotsOption);

	$logoJingles = new Zend_Form_Element_CheckBox('sp_logoJingles');
        $logoJingles->setLabel(_('Logo After Jingles'))
                   ->setDecorators(array('viewHelper'));
        if (isset($storedCrit["logoJingles"])) {
            $logoJingles->setChecked($storedCrit["logoJingles"]["value"] == 1?true:false);
        }
        $this->addElement($logoJingles);

	$logoSpots = new Zend_Form_Element_CheckBox('sp_logoSpots');
        $logoSpots->setLabel(_('Logo After Spots'))
                   ->setDecorators(array('viewHelper'));
        if (isset($storedCrit["logoSpots"])) {
            $logoSpots->setChecked($storedCrit["logoSpots"]["value"] == 1?true:false);
        }
        $this->addElement($logoSpots);
    ...
    public function isValid($params)
    {
		...
		$element = $this->getElement("sp_logo_value");
            if ($data['etc']['sp_logo_value'] == "" || floatval($data['etc']['sp_logo_value']) < 0) {
                $element->addError(_("Cannot be empty or smaller than 0"));
                $isValid = false;
            }

        $element = $this->getElement("sp_jingles_value");
            if ($data['etc']['sp_jingles_value'] == "" || floatval($data['etc']['sp_jingles_value']) < 0) {
                $element->addError(_("Cannot be empty or smaller than 0"));
                $isValid = false;
            }

        $element = $this->getElement("sp_spots_value");
            if ($data['etc']['sp_spots_value'] == "" || floatval($data['etc']['sp_spots_value']) < 0) {
                $element->addError(_("Cannot be empty or smaller than 0"));
                $isValid = false;
            }
